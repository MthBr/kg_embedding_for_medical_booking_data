...
if _local_dir:
    ...
    reportings_dir = Path(__file__).resolve().parents[1].joinpath('data', 'reports')
    log_dir = Path(__file__).resolve().parents[1].joinpath('data', 'log_files')
else:
    data_dir = Path(__file__).resolve().parents[4].joinpath('ECML_2020', 'data')
    reportings_dir = Path(__file__).resolve().parents[4].joinpath('ECML_2020', 'reports')
    log_dir = Path(__file__).resolve().parents[4].joinpath('ECML_2020', 'log_files')
